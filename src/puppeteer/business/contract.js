...
// 打印合同
async function noToYesContract(opts) {
  const {browser, page, invoiceArr} = opts
  const start = Date.now();
  log.info(`----打印销售合同自动化任务开始----`)
  await page.goto('http://cdwp.cnbmxinyun.com/#/app/approval')
  await page.waitForSelector('#table > tbody > tr')
  // 计算行号
  const rowIndex = await page.$$eval('#table > tbody > tr > td:nth-child(2)', (els) => {
  ...
  async function submitOne(href, invoiceNo) {
    ...
    await pageOne.bringToFront()
    await pageOne.setViewport({width: screenWidth, height: screenHeight});
    // await pageOne.goto(`http://cdwp.cnbmxinyun.com/print/printlandscape.html?uri=/contract/printview&contractno=${invoiceNo}&type=c1&processId=${href.match(/processId=([^\&]*)/g)[0].split('=')[1]}&nodeId=${href.match(/nodeId=([^\&]*)/g)[0].split('=')[1]}`)
    await pageOne.goto(`http://cdwp.cnbmxinyun.com/contract/printview?contractno=${invoiceNo}&type=c1&processId=${href.match(/processId=([^\&]*)/g)[0].split('=')[1]}&nodeId=${href.match(/nodeId=([^\&]*)/g)[0].split('=')[1]}`)
    try {
      await pageOne.waitForSelector(`.pageH .contractTitle`)
      log.info(`----${invoiceNo}合同处理成功----`)
      successArr.push(invoiceNo)
    } catch(e) {
      log.error(`----${invoiceNo}合同处理失败----打印环节失败----`)
      errorArr.push(invoiceNo)
    }
    // 成功之后关闭页面
    await pageOne.close()
